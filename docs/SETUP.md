...
「マイグレーション 11件すべて適用済み」「調査項目 31件」と出れば完了。
...
判定数は **31項目 × 校数 × 走査回数**。既定（自校・比較校とも月次・比較4校）で
**月155判定**。この数字は 09 設定画面にその場で表示され、頻度を変えると即座に更新される。

更新の速い時期だけ自校を週次に上げる、といった調整もできる。抑えたい場合は
設定画面から次を調整できる。

| 設定 | 既定 | 効果 |
|---|---|---|
| 自校・比較校の走査頻度 | 月次 | 週次にすると判定数が4倍 |
...
走査の cron（`0 21 * * 0` = 月曜 6:00 JST）は「様子を見に来る」頻度で、
**実際に走査するかどうかは設定画面のスケジュール（既定は自校・比較校とも月次）で決まる**。
様子見の頻度を実際の走査頻度より高くしておくことで、失敗しても数日以内に拾い直せる。
既定のままなら、月に一度スケジュールに達した学校だけが実際に走査される
（他の週は対象0校で数秒で終わる）。

すぐ試すには 09 設定画面の**「今すぐ走査する」ボタン**が手軽（管理者のみ）。
頻度設定を待たず自校・比較校をいま走査する。ただしアプリのサーバ内で
クロールと判定を行うため、比較校が多い組織や Vercel Hobby プランでは
実行時間の上限で途中打ち切りになることがある。その場合は
**Actions → 走査（スケジュール実行）→ Run workflow** を使う
（`dry_run` にチェックを入れると対象の確認だけで走査しない）。

> 設定画面で曜日・実行日・時刻を変えても `scan.yml` の cron を合わせる必要はない。
> 様子見は週1回来るので、スケジュールに達してから最大で数日のうちに拾われる。

**失敗した学校は水曜・金曜に自動で拾い直す。** 前回走査として数えるのは走り切った回
だけなので（`runner.ts` が `status='done'` で絞っている）、失敗した学校は次に様子を
見に来たときも対象に残る。全部成功していれば水曜・金曜は0校で終わる。
...
| | 制限 | 影響と対処 |
|---|---|---|
| Supabase Free | **7日間アクセスが無いとプロジェクトが一時停止** | 走査は既定で月次のため、走査だけでは猶予（7日）に間に合わない。`keepalive.yml` が3日ごとに触って止まらないようにしている。有料プランなら不要（止めてよい） |
...
| GitHub Actions | 公開リポジトリは無料 | 非公開なら月2,000分。ワークフローの様子見自体は週1回（数分）で、実際の走査は月1回程度。十分収まる |
...
### 03 導線の実測のうち、操作・読み取りが要るもの

走査から機械的に出せるのは10指標のうち**更新件数と写真の点数の2つだけ**。
残りは実際に操作する・記事の中身を読む・行事の実施日を確認する必要があり、
機械的には求まらない（`src/lib/analysis/measurements.ts` に指標ごとの理由がある）。

| 指標 | 何が要るか |
|---|---|
| 説明会申込までのクリック数／学校案内を読み始めるまでのクリック数 | 実際に操作した記録 |
| うち学校の様子が伝わる記事の件数 | 記事の内容の読み取り（事務連絡かどうか） |
| お知らせのカテゴリ数 | 分類の呼び方は学校ごとに違うため、機械では数えられない |
| 行事の実施から公開までの日数 | 行事の実施日の読み取り |
| 動画の本数／部活動の個別ページ数 | 走査では数えていない（ページ名から機械的には判別できない） |
| スマートフォンでの表示完了時間 | 外部の測定ツールの結果 |

現時点ではこれらを**画面から入力する口は無い**（04 SE-01 の順位記録のような
入力フォームは無い）。09 設定の測定条件（MS-05）で走査由来か操作由来かは
分かるが、未計測の指標は「未計測」と出るだけで、埋める手段がない。
会議で使う数値が必要なときは、担当者が手元で数えて 06 の照会欄などに書き添える運用になる。
入力フォームを追加すれば自動反映できる形にはしてある（`measurements` テーブルへの
INSERT が増えるだけ）。

### 表示速度

外部の測定ツールに依存し、測るたびに値が変動する。上と同じく現状は未計測のまま出る。
...
**自動実行が動かない**
→ 週1回の自動実行は GitHub Actions（`scan.yml`）が回している（7-3）。
GitHub の **Actions** タブで実行が起きているか、Secrets が4つとも登録されているかを確認する。
09 設定画面の実行記録に「自動」の行が並んでいれば動いている。「手動」しか無ければ
GitHub Actions からではなく手元やほかの経路で実行したことになる。
（`/api/cron/scan` は自前サーバから叩く用の別経路で、`CRON_SECRET` で認証する。
サーバレスの実行時間の上限があるため、週1回の定期実行には使っていない）
