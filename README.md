...
- **走査は `.github/workflows/scan.yml`** が週1回（月曜 6:00 JST）＋失敗時の拾い直し
  （水・金）の頻度で様子を見に来る。実際に走査するかは設定画面のスケジュール
  （既定は自校・比較校とも月次）で決まり、様子見の頻度とは別。様子見のほうを
  実際の走査頻度より高くしておくことで、失敗しても数日以内に拾い直せる。
  Vercel の関数で回さないのは、31項目 × 校数のLLM判定とクロールが
  関数の実行時間上限（Pro でも 800 秒）を超えるため
- **失敗した学校は水曜・金曜に拾い直す。** 前回走査として数えるのは走り切った回
  だけなので（`status='done'` で絞っている）、失敗した学校は次に様子を見に来た
  ときも対象に残る。全部成功していれば拾い直しの回は0校で終わる
...
- **`.github/workflows/keepalive.yml`** が3日ごとにデータベースを読む。
  Supabase の無料プランは7日間アクセスが無いと一時停止するため、
  走査の頻度に関わらず一定間隔で触っておく必要がある。有料プランなら不要
...
| 区分 | 設定できること | 既定値 |
|---|---|---|
| 走査スケジュール | 自校・比較校それぞれの頻度（週次／隔週／月次／手動のみ）、実行曜日・実行日・実行時刻 | 自校・比較校とも月次、1日 6時 |
...
> `scan.yml` の `schedule:` をコメントアウトしてある（Actions タブの
> 「Run workflow」、または09設定画面の「今すぐ走査する」からの手動実行は動く）。
> 再開するにはそのコメントを外して push する。

定期実行は `.github/workflows/scan.yml`（GitHub Actions）が週1回、月曜 6:00（日本時間）に
`npm run scan:due -- --run` を回す。この頻度は「様子を見に来る」頻度で、実際に走査するかは
設定画面のスケジュール（既定は月次）で決まる。様子見の頻度を実際の走査頻度より高くして
おくことで、失敗しても数日以内に拾い直せる。外部から叩く口として `/api/cron/scan` も
用意してあり、CLI・cron・Actions のいずれも同じ `src/lib/scan/runner.ts` を通る。
...
- **A 判定コスト** — 自校と比較校の頻度差は**設定画面で変更できる**ようにし、既定は「自校・比較校とも月次」にした（比較4校で月155判定）。判定コストを最初から抑える運用判断で、更新が速い時期だけ自校を週次に上げる想定。候補ページ抽出で LLM に渡す本文量と思考深度も設定項目にしてある。共通のシステムプロンプトはプロンプトキャッシュ対象。さらに絞る場合は Message Batches API（50%割引）に載せ替えられるよう、1判定=1リクエストの形にしてある
