...
> 実装状況：01〜07 と 08 設定はすべて画面として動く。04 のうち検索順位だけは
> 外部APIの選定が未決のため手動記録にとどめている（後述の「要確定事項 D」）。
...
tests/                        集計・クロール・判定・04・05・06 のテスト
```
...
| 見た目だけでロジック未接続 | 対応済みトグルは `useActionStatus` → `PATCH /api/actions/[id]` に接続済み。01 と 06 で状態が共有されることを確認している。06 の照会欄・05 の生成ボタン・04 の順位記録も同様に接続済みで、鍵が無い場合は押せなくしたうえで理由を出す |
